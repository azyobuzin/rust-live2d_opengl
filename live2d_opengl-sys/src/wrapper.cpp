...
// Live2DModelWinGL
#define self static_cast<Live2DModelWinGL*>(p)

extern "C" {
    void Live2DModelWinGL_setTexture(void *p, int textureNo, unsigned int openGLTextureNo) {
        self->setTexture(textureNo, openGLTextureNo);
    }

    Live2DModelWinGL* Live2DModelWinGL_loadModelFromFile(const char *filepath) {
        LDString pathStr(filepath);
        return Live2DModelWinGL::loadModel(pathStr);
    }

    Live2DModelWinGL* Live2DModelWinGL_loadModelFromBuffer(const void* buf, int bufSize) {
        return Live2DModelWinGL::loadModel(buf, bufSize);
    }

    void Live2DModelWinGL_setMatrix(void *p, float *matrix) {
